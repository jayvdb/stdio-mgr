...
def test_context_manager_instantiation(stdio_mgr):
    ...
    if isinstance(cm, (BufferReplaceStdioManager, BufferInjectStdioManager)):
        assert isinstance(cm.stdout, SafeCloseRandomTextIO)
        assert isinstance(cm.stderr, SafeCloseRandomTextIO)

    if isinstance(cm, FileInjectStdioManager):
        assert isinstance(cm.stdout, SafeCloseRandomFileIO)
        assert isinstance(cm.stderr, SafeCloseRandomFileIO)
